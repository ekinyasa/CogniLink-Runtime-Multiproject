...
<div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem; margin-bottom: 1rem; margin-top: 1.5rem;">
            ...
            <div id="studio-landing-effective-summary" style="background: var(--bg-alt); padding: 1rem; border-radius: var(--radius-sm); border: 1px solid var(--border); font-family: monospace; font-size: 0.8rem; color: var(--text); margin-bottom: 1rem; white-space: pre-wrap;">
              <!-- Populated by JS -->
            </div>
            
            <div style="border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 1rem; display: flex; flex-direction: column; gap: 1rem;">
              <p style="font-weight: bold; margin: 0; color: var(--text); font-size: 0.85rem;">ROUTING OVERRIDES</p>
              
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; align-items: start;">
                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">Redirect if Hot <span id="lbl-badge-dest-hot"></span></div>
                  <input type="text" id="studio-landing-dest-hot" placeholder="Inherit from Intent" />
                </label>
                
                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">Redirect if Warm <span id="lbl-badge-dest-warm"></span></div>
                  <input type="text" id="studio-landing-dest-warm" placeholder="Inherit from Intent" />
                </label>

                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">Redirect if Form Submitted <span id="lbl-badge-dest-converted"></span></div>
                  <input type="text" id="studio-landing-dest-converted" placeholder="Inherit from Intent" />
                </label>
                
                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">Redirect if Converted (Sale) <span id="lbl-badge-dest-sale"></span></div>
                  <input type="text" id="studio-landing-dest-sale" placeholder="Inherit from Intent" />
                </label>
              </div>
            </div>
            
            <div style="border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 1rem; display: flex; flex-direction: column; gap: 1rem; margin-top: 1rem;">
              <p style="font-weight: bold; margin: 0; color: var(--text); font-size: 0.85rem;">SCORING OVERRIDES</p>

              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; align-items: start;">
                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">"Hot" Intent Threshold <span id="lbl-badge-hot-threshold"></span></div>
                  <input type="number" id="studio-landing-hot-threshold" placeholder="Inherit from Intent" min="0" max="100" />
                </label>
                
                <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                  <div style="display:flex; justify-content:space-between;">Form Submit Selector <span id="lbl-badge-conv-selector"></span></div>
                  <input type="text" id="studio-landing-conv-selector" placeholder="Inherit from Intent" />
                </label>
              </div>
              
              <details style="margin-top: 0.5rem; border: 1px dashed var(--border); border-radius: var(--radius-sm); padding: 0.75rem;">
                <summary style="font-size: 0.8rem; font-weight: bold; color: var(--text); cursor: pointer; outline: none;">Advanced Scoring Overrides</summary>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem;">
                  <label style="display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; color: var(--text-m);">
                    <div style="display:flex; justify-content:space-between;">Hard Click Points <span id="lbl-badge-hard-click"></span></div>
            ...
            <details style="margin-top: 1rem;">
              <summary style="font-size: 0.8rem; color: var(--text-m); cursor: pointer; outline: none;">Custom Override JSON <span id="landing-json-error" style="color:var(--danger); display:none; margin-left: 0.5rem;">Invalid JSON!</span></summary>
...
function markIntentUnsaved() {
  document.getElementById('intent-unsaved-badge').style.display = 'inline';
}

function clearIntentUnsaved() {
  document.getElementById('intent-unsaved-badge').style.display = 'none';
}

function markLandingUnsaved() {
  document.getElementById('landing-unsaved-badge').style.display = 'inline';
}

function clearLandingUnsaved() {
  document.getElementById('landing-unsaved-badge').style.display = 'none';
}
...
function syncIntentUiToJson() {
  ...
  const evalCfg = studioCampaignConfig.routing.evaluation;
  
  const hc = document.getElementById("studio-intent-hard-click")?.value;
  if (hc) evalCfg.hardClickPoints = Number(hc); else delete evalCfg.hardClickPoints;
  
  const sc = document.getElementById("studio-intent-soft-click")?.value;
  if (sc) evalCfg.softClickPoints = Number(sc); else delete evalCfg.softClickPoints;
  
  const ht = document.getElementById("studio-intent-hard-time")?.value;
  if (ht) evalCfg.hardTimeSeconds = Number(ht); else delete evalCfg.hardTimeSeconds;
  
  const st = document.getElementById("studio-intent-soft-time")?.value;
  if (st) evalCfg.softTimeSeconds = Number(st); else delete evalCfg.softTimeSeconds;
...
function syncIntentJsonToUi() {
  if (!studioCampaignConfig) return;
  const raw = document.getElementById("studio-routing-config").value.trim();
  const errEl = document.getElementById("intent-json-error");
  if (!raw) {
  ...
  const elHc = document.getElementById("studio-intent-hard-click"); if (elHc) elHc.value = evalCfg.hardClickPoints || "";
  const elSc = document.getElementById("studio-intent-soft-click"); if (elSc) elSc.value = evalCfg.softClickPoints || "";
  const elHt = document.getElementById("studio-intent-hard-time"); if (elHt) elHt.value = evalCfg.hardTimeSeconds || "";
  const elSt = document.getElementById("studio-intent-soft-time"); if (elSt) elSt.value = evalCfg.softTimeSeconds || "";
...
function attachIntentSyncListeners() {
  ...
  const jsonEl = document.getElementById("studio-routing-config");
...
function syncLandingJsonToUi() {
  if (!studioCurrentEditingLanding) return;
  const raw = document.getElementById("studio-landing-override-json").value.trim();
  const errEl = document.getElementById("landing-json-error");
  
  if (!raw) {
    studioCurrentEditingLanding.destinations = {};
    studioCurrentEditingLanding.signals = {};
    errEl.style.display = 'none';
  } else {
    try {
      const parsed = JSON.parse(raw);
      studioCurrentEditingLanding.destinations = parsed.destinations || {};
      studioCurrentEditingLanding.signals = parsed.signals || {};
      errEl.style.display = 'none';
    } catch(e) {
      errEl.style.display = 'inline';
      return;
  ...
  const dh = document.getElementById("studio-landing-dest-hot"); if(dh) dh.value = dests.hot || "";
  const dw = document.getElementById("studio-landing-dest-warm"); if(dw) dw.value = dests.warm || "";
  const dc = document.getElementById("studio-landing-dest-converted"); if(dc) dc.value = dests.converted || "";
  const ds = document.getElementById("studio-landing-dest-sale"); if(ds) ds.value = dests.sale || "";
  
  const hht = document.getElementById("studio-landing-hot-threshold"); if(hht) hht.value = sigs.hotThreshold || "";
  const cs = document.getElementById("studio-landing-conv-selector"); if(cs) cs.value = sigs.conversionSelector || "";
  
  const hc = document.getElementById("studio-landing-hard-click"); if(hc) hc.value = sigs.hardClickPoints || "";
  const sc = document.getElementById("studio-landing-soft-click"); if(sc) sc.value = sigs.softClickPoints || "";
  const ht = document.getElementById("studio-landing-hard-time"); if(ht) ht.value = sigs.hardTimeSeconds || "";
  const st = document.getElementById("studio-landing-soft-time"); if(st) st.value = sigs.softTimeSeconds || "";
...
function syncLandingUiToJson() {
  ...
  if (Object.keys(studioCurrentEditingLanding.signals).length > 0) toSave.signals = studioCurrentEditingLanding.signals;
  
  const el = document.getElementById("studio-landing-override-json");
...
function updateEffectiveSummary() {
  if (!studioCampaignConfig || !studioCurrentEditingLanding) return;
  
  const intentEval = (studioCampaignConfig.routing && studioCampaignConfig.routing.evaluation) || {};
  const intentDest = (studioCampaignConfig.routing && studioCampaignConfig.routing.destinations) || {};
  
  const landingEval = studioCurrentEditingLanding.signals || {};
  const landingDest = studioCurrentEditingLanding.destinations || {};
  ...
  const summEl = document.getElementById('studio-landing-effective-summary');
  if (summEl) {
    let html = `<strong style="color:var(--primary);">Effective Landing Configuration</strong>\n\n`;
    html += `[ SCORING ]\n`;
    html += `Hot Threshold  : ${effectiveEval.hotThreshold || 60} ${landingEval.hotThreshold !== undefined ? '(Overridden)' : '(Inherited)'}\n`;
    html += `Hard Click Pts : ${effectiveEval.hardClickPoints || 30} ${landingEval.hardClickPoints !== undefined ? '(Overridden)' : '(Inherited)'}\n`;
    html += `Soft Click Pts : ${effectiveEval.softClickPoints || 15} ${landingEval.softClickPoints !== undefined ? '(Overridden)' : '(Inherited)'}\n`;
    html += `Max Time Pts   : ${effectiveEval.hardTimeSeconds || 30} ${landingEval.hardTimeSeconds !== undefined ? '(Overridden)' : '(Inherited)'}\n`;
    html += `Min Time Pts   : ${effectiveEval.softTimeSeconds || 10} ${landingEval.softTimeSeconds !== undefined ? '(Overridden)' : '(Inherited)'}\n\n`;
